...
class StableDiffusionDualUNetImprovedPipeline(
    DiffusionPipeline,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        import copy
        self.gm_scheduler = copy.deepcopy(self.scheduler)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # latent_gm_model_input = torch.cat([gm_latents] * 2) if self.do_classifier_free_guidance else gm_latents
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
                gm_latents = self.gm_scheduler.scale_model_input(gm_latents, t)
                # latent_model_input = gm_latents
                ...
                # if t < int(len(timesteps) // 5):
                #     gm_latents = self.gm_scheduler.step(sdr_noise_pred, t, gm_latents, **extra_step_kwargs, return_dict=False)[0]
                    
                # else:
                # tried, it seems that init 20% denoise with sd unet make no change.

                gm_latent_input = torch.cat([x0_latent, gm_latents], dim=1)
        ...
        sdr_latent = self.image_processor.postprocess(image[0], output_type=output_type, do_denormalize=do_denormalize)
        gm_latent = self.image_processor.postprocess(image[1], output_type=output_type, do_denormalize=do_denormalize)
        return (sdr_latent,gm_latent)
        ...
        return StableDiffusionPipelineOutput(images=(sdr_latent,gm_latent), nsfw_content_detected=has_nsfw_concept)