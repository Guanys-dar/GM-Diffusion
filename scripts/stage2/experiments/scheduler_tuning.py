...
from diffusers import DPMSolverMultistepScheduler  # Add this line
...
def main():
    ...
    #     subfolder="scheduler",
    #     # Optional: Adjust these parameters for better performance
    #     beta_start=0.00085,
    #     beta_end=0.012,
    ...
    noise_scheduler = DPMSolverMultistepScheduler.from_pretrained(
        ...
        beta_end=0.012,
        beta_schedule="scaled_linear",
        algorithm_type="dpmsolver++",  # Use DPM-Solver++
        solver_order=2,                # Higher order for better convergence
        prediction_type="epsilon",
        steps_offset=1,
        thresholding=False,            # Disable thresholding for HDR preservation
    )
    ...
    no_resize_val_transforms = transforms.Compose(
        [
            transforms.ToTensor(),
            transforms.Normalize([0.5], [0.5]),
        ]
    ...
    for sdr_input_path in tqdm(sdr_input_paths, desc="Processing SDR images"):
        ...
        record_original_sdr_image = sdr_image.copy()
        original_sdr_image = no_resize_val_transforms(sdr_image.copy()).unsqueeze(0).to(device)
        original_sdr_image = original_sdr_image.half()
        print("before resize:", record_original_sdr_image.size)
        sdr_image = val_transforms(sdr_image).unsqueeze(0).to(device)
        sdr_image = sdr_image.half()
        ...
        with torch.no_grad():
            ...
            # return a vae decode sdr image
            orginial_sdr_latent = pipeline.vae.encode(original_sdr_image).latent_dist.sample()
            orginial_sdr_latent = orginial_sdr_latent * pipeline.vae.config.scaling_factor
            orginial_sdr_latent = 1 / vae.config.scaling_factor * orginial_sdr_latent
            original_sdr_decoded = pipeline.vae.decode(orginial_sdr_latent, return_dict=False)[0]
            original_sdr_decoded = (original_sdr_decoded / 2 + 0.5).clamp(0, 1)
            original_sdr_decoded = original_sdr_decoded.cpu().permute(0, 2, 3, 1).float().numpy()[0]
        ...
        sdr_image_decoded = cv2.resize(sdr_image_decoded, (record_original_sdr_image.size[0], record_original_sdr_image.size[1]), interpolation=cv2.INTER_LINEAR)
        gm_image = cv2.resize(gm_image, (record_original_sdr_image.size[0], record_original_sdr_image.size[1]), interpolation=cv2.INTER_LINEAR)
        original_sdr_decoded = cv2.resize(original_sdr_decoded, (record_original_sdr_image.size[0], record_original_sdr_image.size[1]), interpolation=cv2.INTER_LINEAR)
