...
def main():
    ...
    seed = 4369
    steps_list = [25, 50, 100, 250, 500, 1000]  # <- Added steps list
    ...
    for prompt in prompts:
        ...
        with torch.no_grad():
            ...
            for steps in steps_list:  # <- Added steps loop
            # Create step-specific output directory
                step_dir = os.path.join(args.output_dir, f"steps_{steps}")
                ...
                sdr_latent, gm_latent = pipeline(
                    ...
                    num_inference_steps=steps,  # Increased from 50
                ...
                # 保存 SDR 和 GM 图像
            # Save images in step-specific directory
                sdr_image_path = os.path.join(step_dir, f"sdr_{sdr_input_filename}")
                gm_image_path = os.path.join(step_dir, f"gm_{sdr_input_filename}")
