"""Stage 1: LoRA fine-tuning for the VQGAN encoder/decoder used in GM Diffusion."""
...
from gm_diffusion.stage1 import (
    Discriminator,
...
def save_hdr_image(apply_HDR, save_dir, i, qmax):
    apply_HDR = apply_HDR / (qmax+1)
    apply_HDR_image = apply_HDR.permute(0, 2, 3, 1).squeeze(0).cpu().numpy()
    apply_HDR_image = apply_HDR_image.astype(np.float32)
    cv2.imwrite(os.path.join(save_dir, f"apply_HDR_{i}.hdr"), apply_HDR_image[:,:,[2,1,0]])
...
def log_validation(
    ...
    """
    Logs and saves validation images generated by the VAE model.

    Args:
        vae (nn.Module): The Variational Autoencoder model.
        args: Arguments containing configuration parameters.
        validation_transform (transforms.Compose): Transformations to apply to validation images.
        accelerator (Accelerator): Accelerator for handling device and mixed precision.
        global_step (int): Current training step.

    Returns:
        List[Image.Image]: List of concatenated original, generated, and processed images.
    """
    logger.info("Starting validation image generation...")
    ...
    dtype = dtype_map.get(accelerator.mixed_precision, torch.float32)

    # Define validation images directory
    val_images_dir = Path(args.val_images_dir)
    if not val_images_dir.is_dir():
        raise ValueError(f"Validation images directory '{args.val_images_dir}' does not exist or is not a directory.")

    # Gather list of PNG images
    image_list = sorted(val_images_dir.glob("*.png"))
    if not image_list:
        raise ValueError(f"No PNG images found in the validation directory '{args.val_images_dir}'.")

    original_images = []
    device = accelerator.device

    logger.info(f"Found {len(image_list)} validation images.")
    ...
    for image_path in image_list:
        with Image.open(image_path) as img:
            if img.mode != "RGB":
                img = img.convert("RGB")
            image_tensor = validation_transform(img).to(device, dtype=dtype)
            original_images.append(image_tensor.unsqueeze(0))  # Add batch dimension

    # Prepare directories for saving outputs
    save_dir = Path(args.output_dir) / "validation_images" / f"step_{global_step}"
    save_dir.mkdir(parents=True, exist_ok=True)
    log_file = save_dir / "evaluation_log.txt"
    ...
    # Aggregate log messages to minimize I/O operations
    log_messages = []

    for idx, original_image in enumerate(original_images):
        # Encode the original image
        latent_dist = unwrapped_vae.encode(original_image).latent_dist
        sampled_latent = latent_dist.sample() * 0.18215

        # Decode the latent representation
        decoded_sample = unwrapped_vae.decode(sampled_latent / 0.18215).sample

        # Log decode output range
        # decode_max = decoded_sample.max().item()
        # decode_min = decoded_sample.min().item()
        # log_messages.append(f"(Evaluation) Decode Output Range: {decode_max}, {decode_min}\n")

        # Apply sigmoid activation
        image = torch.sigmoid(decoded_sample)
        sigmoid_max = image.max().item()
        sigmoid_min = image.min().item()
        log_messages.append(f"(Evaluation) Sigmoid Output Range: {sigmoid_max}, {sigmoid_min}\n")

        # Apply HDR to SDR transformation
        apply_hdr = apply_gm_to_sdr(gm=image, sdr=(original_image + 1) / 2, qmax=49)
        print(apply_hdr.max(),apply_hdr.min())
        hdr_max = apply_hdr.max().item()
        hdr_min = apply_hdr.min().item()
        log_messages.append(f"(Evaluation) Apply HDR Range: {hdr_max}, {hdr_min}\n")

        # Append generated images
        generated_images.append(image)

        # Save HDR image
        save_hdr_image(apply_hdr, save_dir, idx, qmax=49)

        # Apply Tone Mapping Operator (TMO) using CUDA
        apply_image = tmo_cuda(apply_hdr)
        processed_images.append(apply_image)

    # Write all log messages at once
    ...
    # Scale images to [0, 255] and convert to NumPy arrays
    original_images_np = (original_images * 255.0).permute(0, 2, 3, 1).cpu().numpy().astype(np.uint8)
    generated_images_np = (generated_images * 255.0).permute(0, 2, 3, 1).cpu().numpy().astype(np.uint8)
    processed_images_np = (processed_images * 255.0).permute(0, 2, 3, 1).cpu().numpy().astype(np.uint8)

    # Concatenate images side by side (Original | Generated | Processed)
    concatenated_images = [
        Image.fromarray(np.concatenate([orig, gen, proc], axis=1))
        for orig, gen, proc in zip(original_images_np, generated_images_np, processed_images_np)
    ]
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            ...
            np_images = np.stack([np.asarray(img) for img in concatenated_images])
            tracker.writer.add_images("validation", np_images, global_step, dataformats="NHWC")
            logger.info("Logged images to TensorBoard.")
        elif tracker.name == "wandb":
            # Prepare images for Weights & Biases
            wandb_images = [
                wandb.Image(img, caption=f"Step {global_step}: Original | Generated | Processed")
                for img in concatenated_images
            ]
            tracker.log({"validation": wandb_images}, step=global_step)
            logger.info("Logged images to Weights & Biases.")

    # Save concatenated images to disk
    for idx, image in enumerate(concatenated_images):
        image_path = save_dir / f"image_{idx}.png"
        image.save(image_path)
    logger.info(f"Saved {len(concatenated_images)} concatenated images to '{save_dir}'.")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=None,
        help="One or more parquet files produced during stage 1 dataset generation.",
    )
    parser.add_argument(
        "--dataset_cache_dir",
        type=str,
    ...
    parser.add_argument(
        "--debug_mode", action="store_true", help="Whether or not to use xformers."
    )

    parser.add_argument("--bright_tmo", type=str)
    parser.add_argument(
...
def choose_tmo(args):
    tmo_map = {
        "hard_clip_tmo": hard_clip_tmo,
        "linear_scale_tmo": linear_scale_tmo,
        "fix_mulog_tmo": fix_mulog_tmo,
    }
    try:
        return tmo_map[args.bright_tmo]
    except KeyError as exc:
        raise ValueError(f"Unknown TMO: {args.bright_tmo}") from exc
...
def main():
    ...
    training_tmo_cuda = choose_tmo(args)
    ...
    if not args.pretrained_model_name_or_path:
        raise ValueError("--pretrained_model_name_or_path must be provided to load the base VAE.")
    ...
    # print_trainable_parameters(vae.encoder)
    # print_trainable_parameters(vae.decoder)

    if args.use_ema:
        # ema_model = EMAModel(model.parameters(), model_cls=VQModel, model_config=model.config)
        ema_model = EMAModel(vae.parameters())
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                if args.use_ema:
                    ...
                    torch.save(ema_model.state_dict(), os.path.join(output_dir, "gm_decoder_ema.pth")) 
                vqmodel = models[0]
    ...
    lora_layers = filter(lambda p: p.requires_grad, vae.parameters())
    ...
    if not args.train_metadata:
        raise ValueError("--train_metadata must point to at least one parquet file produced in stage 1.")
    data_files = args.train_metadata if len(args.train_metadata) > 1 else args.train_metadata[0]
    dataset = load_dataset("parquet", data_files=data_files, split="train", cache_dir=args.dataset_cache_dir)
    ...
    vae, discriminator, optimizer, discr_optimizer, lr_scheduler, discr_lr_scheduler = accelerator.prepare(
        vae, discriminator, optimizer, discr_optimizer, lr_scheduler, discr_lr_scheduler
    )
    if args.use_ema:
        ema_model.to(accelerator.device)
    # Train!
    ...
    logger.info("***** Running training *****")
    print_trainable_parameters(vae)
    # print_trainable_parameters(vae.module.encoder)
    # print_trainable_parameters(vae.module.decoder)

    if args.clip_pixel:
        prob = 0.7
        clip_pixel = RandomExposureAdjust(gamma=2.2, prob=prob)
        logger.info(f"Using exposure clip!, prob: {prob}")
    else:
        logger.info(f"Not using exposure clip!")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            # apply exposure clip
            if args.clip_pixel:
                to_be_clip = (pixel_values.clone() + 1) / 2
                clipped = clip_pixel(to_be_clip)
                miss_pixel_values = (clipped * 2) - 1
            else:
                miss_pixel_values = pixel_values

            pixel_values = pixel_values.to(accelerator.device, non_blocking=True)
            miss_pixel_values = miss_pixel_values.to(accelerator.device, non_blocking=True)
            ...
            sdr_latent = vae.encode(miss_pixel_values).latent_dist.sample().mul_(0.18215)
            fmap_gm = vae.decode(sdr_latent / 0.18215).sample
            fmap_gm = torch.nn.functional.sigmoid(fmap_gm)
            
            sdr_image = (miss_pixel_values+1)/2
            target = (pixel_values+1)/2
            # logger.info(f"gm range: {fmap_gm.max()}, {fmap_gm.min()}")

            assert torch.all(fmap_gm >= 0) and torch.all(fmap_gm <= 1), f"Training, gm range: {fmap_gm.max()}, {fmap_gm.min()}"
            assert torch.all(sdr_image >= 0) and torch.all(sdr_image <= 1), f"Training, sdr_image range: {pixel_values.max()}, {pixel_values.min()}"
            assert torch.all(target >= 0) and torch.all(target <= 1)

            qmax = 49
            fmap_hdr = apply_gm_to_sdr(gm=fmap_gm, sdr=sdr_image, qmax=qmax)
            # print(fmap_hdr.max(),fmap_hdr.min())

            # if args.tmo_2446a:
            #     tmo_fmap_hdr = RGB_tmo_2446a(fmap_hdr/10)
            # else:
            tmo_fmap_hdr = training_tmo_cuda(fmap_hdr, qmax=qmax)
            tmo_fmap_hdr = gamut_compress(tmo_fmap_hdr)

            if args.debug_mode and i % 50 == 0:
                debug_dir = os.path.join(args.output_dir, f"debug_train")
                os.makedirs(debug_dir, exist_ok=True) 
                concat_image = torch.cat((sdr_image, fmap_gm, fmap_hdr, tmo_fmap_hdr, target), dim=-1)
                torchvision.utils.save_image(concat_image[:4,...], os.path.join(debug_dir, f"step_{i}_concat_image.png"), nrow=1)

            if generator_step:
                with accelerator.accumulate(vae):
                    # reconstruction loss. Pixel level differences between input vs output
                    if args.vae_loss == "l2":
                        loss = F.mse_loss(target, tmo_fmap_hdr)
                    else:
                        loss = F.l1_loss(target, tmo_fmap_hdr)
                    ...
                    # generator loss
                    gen_loss = -discriminator(tmo_fmap_hdr).mean()
                    last_dec_layer = accelerator.unwrap_model(vae).decoder.conv_out.weight
                    # print(vae.decoder.conv_out.weight.requires_grad)
                    # print(f"last_dec_layer.requires_grad: {last_dec_layer.requires_grad}")
                    # assert stop
                    ...
                    # if args.non_zero_loss:
                        # logger.info("Use non zero loss!")
                        # loss += 0.01 * torch.exp(-fmap_gm).mean()
                    ...
                    if (
                        ...
                        log_grad_norm(vae, accelerator, global_step)
            else:
                # Return discriminator loss
                with accelerator.accumulate(discriminator):
                    tmo_fmap_hdr.detach_()
                    target.requires_grad_()
                    real = discriminator(target)
                    fake = discriminator(tmo_fmap_hdr)
            ...
            if accelerator.sync_gradients and not generator_step and accelerator.is_main_process:
                ...
                if global_step % args.validation_steps == 0:
                    if args.use_ema:
                        ...
                        ema_model.store(vae.parameters())
                        ema_model.copy_to(vae.parameters())
                    log_validation(vae, args, validation_transform, accelerator, global_step)
                    if args.use_ema:
                        # Switch back to the original VQGAN parameters.
                        ema_model.restore(vae.parameters())
            end = time.time()
    ...
    if accelerator.is_main_process:
        ...
        if args.use_ema:
            ema_model.copy_to(vae.parameters())
        vae.save_pretrained(os.path.join(args.output_dir, "finetuned_VAE"))
